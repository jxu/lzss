...
LZSS specifically uses a one-bit flag to encode whether an output "token" is a literal byte or a reference. To keep byte-addressed operations efficient, the flags are packed into 8 flags in one byte, then the next 8 tokens follow. This lets literal bytes be mixed freely with references without needing further encoding. 

It can't be the most efficient to use 1 bit for every single literal and reference, as compressed text data I've looked at seems to be mostly runs of references, which could be run-length encoded, with literals interspersed in. Additionally, offsets tend to be quite large and lengths tend to be quite short. DEFLATE uses Huffman coding after LZ77, which could take advantage of this imbalance. Most of the compression in DEFLATE is from LZ77, but Huffman contributes a sizeable minority, maybe 10-30%. 
